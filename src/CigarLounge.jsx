...
{/* RATINGS */}
{activeTab==="ratings"&&<><Header title="All Ratings" subtitle={`${allRated.length} rated`}/><div style={{padding:"12px 20px"}}><input style={{width:"100%",background:"#1a1510",border:"1px solid #2a2318",borderRadius:10,padding:"10px 14px",color:"#e8dcc8",fontSize:14,outline:"none",boxSizing:"border-box"}} placeholder="🔍 Search..." value={searchQuery} onChange={e=>setSearchQuery(e.target.value)}/></div><div style={{padding:"0 20px",display:"grid",gap:12}}>{filtered(allRated).length===0?<div style={{textAlign:"center",padding:"40px 0"}}><span style={{fontSize:48}}>⭐</span><p style={{color:"#6b5e4f"}}>No ratings yet</p></div>:filtered(allRated).sort((a,b)=>getAvgRating(b.ratings)-getAvgRating(a.ratings)).map(c=><CigarCard key={c.id} cigar={c} onTap={setViewCigar} onFavorite={handleToggleFavorite} onShare={setShareCigar} isFavorite={favorites.has(c.id)}/>)}</div></>}
...
{// Pending Request - inline since it needs pendingProfiles from parent
